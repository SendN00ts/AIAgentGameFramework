...
declare class GameClientV2 implements IGameClient {
    private apiKey;
    private llmModel;
    private llmModelBaseUrl;
    private llmModelApiKey;
    client: Axios;
    private baseUrl;
    constructor(apiKey: string, llmModel: LLMModel | string, llmModelBaseUrl: string, llmModelApiKey: string);
    createMap(workers: GameWorker[]): Promise<Map>;
    createAgent(name: string, goal: string, description: string): Promise<GameAgent$1>;
    getAction(agentId: string, mapId: string, worker: GameWorker, gameActionResult: ExecutableGameFunctionResponseJSON | null, environment: Record<string, any>, agentState: Record<string, any>): Promise<GameAction>;
    setTask(agentId: string, task: string): Promise<string>;
    getTaskAction(agentId: string, submissionId: string, worker: GameWorker, gameActionResult: ExecutableGameFunctionResponseJSON | null, environment: Record<string, any>): Promise<GameAction>;
    createChat(data: Record<string, any>): Promise<string>;
    updateChat(conversationId: string, data: Record<string, any>): Promise<GameChatResponse>;
    reportFunction(conversationId: string, data: Record<string, any>): Promise<GameChatResponse>;
    endChat(conversationId: string, data: Record<string, any>): Promise<GameChatResponse>;
}
...
declare class Chat {
    ...
    private actionSpace;
    private getStateFn;
    constructor(chatId: string, client: GameClientV2, actionSpace?: GameFunction<any>[], getStateFn?: () => Record<string, any>);
    next(message: string): Promise<ChatResponse>;
    end(message?: string): void;
    updateConversation(message: string): Promise<GameChatResponse>;
    reportFunctionResult(result: ExecutableGameFunctionResponse, fnId: string): Promise<string>;
}
...
export { Chat, ChatAgent, ExecutableGameFunctionResponse, ExecutableGameFunctionStatus, GameAgent, GameFunction, GameWorker, LLMModel };
